    ...
    public Integer resetPassword(Integer operatorId, ResetPasswordBean resetPasswordBean) {
        ...
        switch (resetPasswordBean.getFlag().getCode()){
            case 1:
                ...
                if (resetPasswordBean.getNewPassword().length() >= 6 && resetPasswordBean.getNewPassword().length()<=12 &&
                        resetPasswordBean.getNewPassword().matches(RegularExpressionDictionary.PASSWORD_REGEX)){
                    ...
                    //resetPasswordBeanResponse.setNewPassword(ResponseCode.PASSWORD_FORMAT_ERROR_MSG);
                    throw new MembraneException(ResponseCode.PASSWORD_SCHEMA_INCORRECT);
                }

            case 0 :
                //重置密码
                // 更新用户密码
                userManagementMapper.updatePassword(resetPasswordBean.getUserId(), Sha256.getSHA256(ResponseCode.RESET_PASSWORD), operatorId);
