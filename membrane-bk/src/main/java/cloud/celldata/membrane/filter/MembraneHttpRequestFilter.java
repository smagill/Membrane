...
public class MembraneHttpRequestFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        //放行swagger
        if (!StringUtils.isEmpty(requestURI) && requestURI.startsWith("/webjars")|| requestURI.startsWith("/swagger-resources")
                ||requestURI.startsWith("/v2") ||requestURI.startsWith("/swagger-ui")){
            filterChain.doFilter(requestReaderHttpServletRequestWrapper, response);
            return;
        }
