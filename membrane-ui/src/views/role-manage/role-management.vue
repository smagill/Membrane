  ...
  <div id="role-manage" :style="{minHeight: minHeight + 'px'}">
    <!-- 条件筛选查询 -->
      ...
      <!-- 角色列表 -->
    	<div class="table-container">
        			...
        			<el-table :data="tab_data">
        					<el-table-column prop="roleName" min-width="205" label="角色名称"></el-table-column>
        					...
        					<el-table-column prop="functionAuthority1" min-width="270" label="功能权限"></el-table-column>
        					<el-table-column min-width="200" label="操作">
        							<template slot-scope="scope">
        									<el-tooltip class="item" effect="dark" content="详情" placement="top">
	        									<em class="iconfont icon-scan" @click="detailClick(scope.row.roleId)"></em>
	        								</el-tooltip>
      ...
      <el-dialog class="plusProModel" title="新增角色" :visible.sync="addDialog" width="746px" :before-close="resetForm" :top="addForm | newDialogTop">
            ...
            <!-- 数据权限渲染 -->
	          <el-form-item v-if="addForm.appPlatform&&addForm.dateAll&&addForm.dateAll.length" label="数据权限" prop="dateLimit" label-width="80px">
      ...
      <div v-if="editDialog">
      		 <el-dialog class="plusProModel" title="编辑角色" :visible.sync="editDialog" width="746px" :before-close="cancelForm" :top="editForm |editDialogTop">
			          ...
			          <!-- <el-form-item v-if="editForm.dataAuthority&&editForm.dataAuthority.length"  label="数据权限" required label-width="80px">
			          	<el-radio-group v-model="editForm.allDataAuthority" size="mini">
			              <el-radio :label= true>全部权限</el-radio>
			              <el-radio :label=false>选择部分权限</el-radio>
			            </el-radio-group>
			            <el-transfer v-if= "!editForm.allDataAuthority" v-model="editForm.dataAuthorityIds" :data="editForm.haveDataAuthority" :titles="['未添加数据权限', '已添加数据权限']"></el-transfer>
      ...
      <el-dialog class="plusProModel" id="detailDialog" title="角色详情" :visible.sync="detailDialog" width="746px" :before-close="cancelDetail" top="30vh">
		  	<el-scrollbar style="height: 300px;padding-right: 2px;">
		  			...
		  			<!-- <div v-if="detailData.dataAuthority.length" class="detailFont"><span style="font-weight: bold;">数据权限：</span>
		  				<div class="detailDatas">
		  					<span v-for="(item,index) in detailData.dataAuthority" :key="index">{{(index == detailData.dataAuthority.length-1) ? item.label:item.label+'、'}}</span>
		  				</div>
		  			</div> -->
		  			<div v-if="detailData.functionAuthorityBean.functionAuthority.length" class="detailFont">
		  				<span class="span" style="font-weight: bold;">功能权限：</span>
  						<el-tree
  							:class="detailData.functionAuthorityBean.functionAuthorityBean == '全部权限' ? 'clearCheckbox':''"
...
export default {
  ...
  methods: {
      ...
      updateRoleHandle(formName){
        ...
        let functionAuthorityIds = [];
	    	...
	    	let params = {
			    ...
			    // extraList: this.addForm.extraList
	    	};
      ...
      addRoles(form){
        ...
        console.log(this.functionAuthorityIds);
	    	let params = {
	    		allDataAuthority: this.addForm.dateLimit,
			    allFunctionAuthority: this.addForm.funcLimit,
			    clientId: this.addForm.clientId,
