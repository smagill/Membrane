<template>
  <div id="user-manage" :style="{minHeight: minHeight + 'px'}">
    ...
    <div class="table-container">
      <template>
        <el-table :data="tab_data">
          <el-table-column min-width="170" prop="userName" label="用户名"></el-table-column>
          <el-table-column min-width="140" prop="fullName" label="姓名"></el-table-column>
          ...
          <el-table-column label="角色" prop="role" align="center" min-width="260">
            <template slot-scope="scope" v-if="scope.row.role">
              <span v-if="scope.row.role.length>20">{{ scope.row.role.substr(0,20)+'...' }}</span>
            ...
            <template slot-scope="scope">
              <span class="spanBtn">
                <el-tooltip class="item" effect="light" content="详情" placement="top">
                  <em class="iconfont icon-scan" @click="detailsHandle(scope.row)"></em>
                </el-tooltip>
              </span>
              <span class="spanBtn">
              ...
              <span class="spanBtn2">
                <el-tooltip class="item" effect="light" content="重置密码" placement="top">
                  <em class="iconfont icon-mima" @click="resetHandle(scope.row)"></em>
                </el-tooltip>
              </span>
            </template>
          </el-table-column>
      ...
      <el-scrollbar style="height:540px">
        <el-form :model="addForm" ref="addForm" :rules="userTest">
            ...
            <el-form-item class="addPhone" label="密码" prop="pwd" label-width="71px">
              <el-input v-model="addForm.pwd" placeholder="请输入6-12位包含字母、数字两者都有的有效密码" size="mini" show-password></el-input>
            </el-form-item>
            ...
            <el-form-item label="邮箱" prop="email" label-width="71px" >
              <el-input v-model="addForm.email" placeholder="如需使用邮箱相关服务，请填写邮箱邮箱" size="mini"></el-input>
            </el-form-item>
            <el-form-item label="角色" label-width="61px" style="margin-left: 10px;">
              <div
                ...
                :key="index"
              >
                <el-checkbox @change="checkChange(item)" v-model="item.checked">{{item.clientName}}</el-checkbox>
    ...
    <el-dialog
      ...
      <el-scrollbar class="editUser" ref="editUser" style="height:540px">
        <el-form :model="editForm" ref="editForm" :rules="userTest">
          ...
          <el-form-item label="姓名" prop="fullName" label-width="71px">
            <el-input maxlength="50" v-model="editForm.fullName" placeholder="请输入姓名" size="mini"></el-input>
          </el-form-item>
          <el-form-item class="addPhone" label="手机号码" prop="telephone" label-width="71px">
          ...
          <el-form-item label="角色" label-width="71px">
              <div
                id="transferDiv"
              ...
              >
                ...
                <div
                  v-if="item.checked==true && item.checkedValue.length==0 && item.chuanSuoYanZheng"
                  class="emptyDate"
                >请至少添加一个角色</div>
              </div>
          </el-form-item>
    ...
    <el-dialog
      ...
      <p
        class="resetP"
      >{{forStartText.status.code==2? '确定启用该用户？':'禁用后用户将无法登录科技系统AI+的所有平台，确定立即禁用？'}}</p>
      <span slot="footer" class="dialog-footer">
      ...
      applicationList(param).then(res=>{
          ...
          if(self.appList.length > 0){
            self.appList.map(item =>{
               ...
               */
              let param = {clientId : item.clientId};
              getSelectedRole(param).then(res=>{
                res.data.map(jtem =>{
                  let objs = {key: jtem.roleId,label:jtem.roleName};
                  obj.clientRoleList.push(objs);
                })
              })
              self.addForm.plusRoleData.push(obj);
              self.roleEditData.push(obj);
            })
          }
        })
    },
